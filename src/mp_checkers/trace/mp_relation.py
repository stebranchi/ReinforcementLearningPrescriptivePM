...
def mp_responded_existence_checker(trace, done, a, b, activation_rules, correlation_rules):
    ...
    if not done and num_violations_in_trace > 0:
    ...
    elif not done and num_violations_in_trace == 0 and num_pendings_in_trace == 0 and num_activations_in_trace > 0:
        ret = True
    elif not done and num_violations_in_trace == 0 and num_pendings_in_trace == 0 and num_activations_in_trace == 0:
        ...
        ret = False
    elif done and num_violations_in_trace == 0 and num_activations_in_trace > 0:
...
def mp_alternate_response_checker(trace, done, a, b, activation_rules, correlation_rules):
    ...
    for event in trace:
        ...
        if event["concept:name"] == b and pending:
            A = pending
            T = event
            if eval(correlation_rules):
                pending = None
    ...
    if not done and num_violations_in_trace == 0 and num_pendings_in_trace > 0:
        ...
        state = TraceState.VIOLATED
        ret = False
    elif done and num_violations_in_trace == 0 and num_pendings_in_trace == 0 and num_activations_in_trace > 0:
        state = TraceState.SATISFIED
        ret = True
...
def mp_alternate_precedence_checker(trace, done, a, b, activation_rules, correlation_rules):
    ...
    state = None
    if not done and num_violations_in_trace == 0 and num_activations_in_trace > 0:
        ret = True
    elif not done and num_violations_in_trace == 0 and num_activations_in_trace == 0:
        state = TraceState.POSSIBLY_SATISFIED
    elif num_violations_in_trace > 0:
        state = TraceState.VIOLATED
        ret = False
