...
def main():
	reward_file = os.path.join("..", "data", "mdp", "BPI_2012_MDP_r_distribution.csv")
	policy_file = os.path.join("..", "data", "mdp", "BPI_2012_MDP_r3_Q_3_zero_q.csv")
	...
	for i in range(50):
		...
		value = np.random.normal(20000, 4000)
	...
	while not timeline.empty():
		...
		for action, _ in actions:
			...
			for possible_next_state in list_of_possibilities.keys():
				...
				if summed_probability < choice and choice <= summed_probability + next_state_probability and possible_next_resource_available is False:
					...
					# With distributions
					if list_of_possibilities[next_state][2] == 0.0:
						reward = 0.0
					elif list_of_possibilities[next_state][1] == "Normal":
						reward = -1 * np.random.normal(list_of_possibilities[next_state][2], list_of_possibilities[next_state][3])
					else:
						reward = -1 * np.random.exponential(list_of_possibilities[next_state][2])
					if reward > 0:
						reward = -1 * reward
		...
		if '<END>' in next_state:
			completed.append((next_state, timestamp - reward - start))
			...
			if next_state == state:
				...
				runs[i].append((next_state, timestamp))
			state = next_state
	...
	create_log(runs, "../data/generated_logs/test_sepsis_1000_500_100.xes")
	test_timestamps(runs)
	#print(completed)
...
def get_rules(MDP_policy_val, MDP_reward_val, random_policy):
	...
	for rowid, row in enumerate(MDP_reward_val):
		...
		if state in states_list:
			...
			# With distributions
			next_state_reward_dist = MDP_reward_val[rowid, 4]
			next_state_reward_mean = MDP_reward_val[rowid, 5]
			next_state_reward_std = MDP_reward_val[rowid, 6]
			reward_dict[state][state_action][next_state] = [next_state_probability, next_state_reward_dist, next_state_reward_mean, next_state_reward_std]
			# without distribuitions
...
def test_timestamps(runs):
	print("testing_timestamps")
...
def create_log(runs, path):
	...
	for i, run in runs.items():
		...
		for (action, timestamp) in run:
			...
			days_to_add = int(timestamp / 28800)
			seconds_to_add = int(timestamp % 28800)
			new_date = today + datetime.timedelta(days=days_to_add) + datetime.timedelta(seconds=seconds_to_add)
			if new_date.time().hour > 18:
				new_date = new_date + datetime.timedelta(days=1) - datetime.timedelta(hours=8)
			elif new_date.time().hour < 8:
