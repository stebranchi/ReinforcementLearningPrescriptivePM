...
from Timeline_train import select_next_action
...
def main():
	...
	output_file = os.path.join("..", "data", "output", "test_states+resources_Q_all_resources.csv")
	...
	for i in range(1, number_of_runs):
		...
		while not timeline.empty():
			...
			if wait:
				...
				next_res_state = next_state
				next_resources = [x for x in resources_per_event[next_state].keys()]
				if len(next_resources) > 0:
					next_res_state += "- " + " ".join([x for x in resources_per_event[next_state].keys()])
				#try:
				if next_state == last_state:
					...
					#next_action = select_action(action_lists, q_table, next_res_state, epsilon)
			...
			# return_dict = add_bpi_2012_reward(path, return_dict)
